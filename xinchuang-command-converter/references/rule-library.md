...
CentOS 使用 `yum` / `dnf`，统信 UOS、银河麒麟的软件源基于 Debian 体系，使用 `apt`：
...
| 原包名 | 转换后 | 说明 |
| --- | --- | --- |
| `redis` | `redis-server` | Debian/统信/麒麟软件源中的 Redis 服务包 |
...
- Redis：优先官方多架构高版本镜像（如 `redis:7-alpine`），或使用统信/麒麟软件源中的 Redis 包。
...
1. 在 `package_manager`、`commands`、`package_names` 中追加正则映射。
...
新增规则后运行 `python scripts/convert_xinchuang.py <样例脚本>` 验证输出与报告。
